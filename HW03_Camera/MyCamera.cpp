...
using namespace BTX;
//  MyCamera
// sets the position, target and above variables and calls CalculateView() to update the view
void MyCamera::SetPositionTargetAndUpward(vector3 a_v3Position, vector3 a_v3Target, vector3 a_v3Upward)
{
	...
	// set position, target and above to given values
...
void MyCamera::MoveForward(float a_fDistance)
{
	...
	// move forwards and backwards with orientation factored in
	m_v3Position += glm::mat3(glm::yawPitchRoll(m_v3PitchYawRoll.y, m_v3PitchYawRoll.x, m_v3PitchYawRoll.z)) * vector3(0.0f, 0.0f, -a_fDistance);
	m_v3Target += glm::mat3(glm::yawPitchRoll(m_v3PitchYawRoll.y, m_v3PitchYawRoll.x, m_v3PitchYawRoll.z)) * vector3(0.0f, 0.0f, -a_fDistance);
	m_v3Above += glm::mat3(glm::yawPitchRoll(m_v3PitchYawRoll.y, m_v3PitchYawRoll.x, m_v3PitchYawRoll.z)) * vector3(0.0f, 0.0f, -a_fDistance);
}

// moves the camera up and down
void MyCamera::MoveVertical(float a_fDistance)
{
	// move vertically without orientation factored in
...
{
	// move to the side with orientation factored in
	m_v3Position += glm::mat3(glm::yawPitchRoll(m_v3PitchYawRoll.y, m_v3PitchYawRoll.x, m_v3PitchYawRoll.z)) * vector3(a_fDistance, 0.0f, 0.0f);
	m_v3Target += glm::mat3(glm::yawPitchRoll(m_v3PitchYawRoll.y, m_v3PitchYawRoll.x, m_v3PitchYawRoll.z)) * vector3(a_fDistance, 0.0f, 0.0f);
	m_v3Above += glm::mat3(glm::yawPitchRoll(m_v3PitchYawRoll.y, m_v3PitchYawRoll.x, m_v3PitchYawRoll.z)) * vector3(a_fDistance, 0.0f, 0.0f);
}
...
void MyCamera::CalculateView(void)
{
	...
	// gets the orientation from m_v3PitchYawRoll
	glm::mat3 orientation = glm::mat3(glm::yawPitchRoll(m_v3PitchYawRoll.y, m_v3PitchYawRoll.x, m_v3PitchYawRoll.z));

	// make the camera view the new view matrix
	m_m4View = glm::lookAt(m_v3Position, m_v3Position + orientation * vector3(0.0f, 0.0f, -1.0f), orientation * vector3(0.0f, 1.0f, 0.0f));
}
